...
async function handleTokenRefresh(error: { config: AxiosRequestConfig }){
    ...
    if(tokenRefresh){
        ...
        try {
            ...
            error.config.headers.Authorization = ApiService.defaults.headers.Authorization;
...
export function ApiServiceHateoas(
    links: ApiLinksInterface[] = [],
    name: string,
    onCanRequest: (request: <T>(data?: AxiosRequestConfig) => Promise<AxiosResponse<T>>) => void,
